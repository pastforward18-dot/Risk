...
st.caption("iPad · Safari 안정화 v2 / 원화·달러 현금 / 전체종목 Correlation 진단 / Sharpe / Risk Parity")
...
with tabs[2]:
    ...
    st.caption(
        "한국/미국 휴장일 차이 때문에 종목이 빠지는 문제를 줄이기 위해, "
        "각 종목쌍의 공통 거래일을 맞춘 뒤 최근 관측치로 계산합니다."
    )
    ...
    with st.expander("🔎 종목별 데이터 상태 확인", expanded=False):
        ...
        for _, row in stocks.iterrows():
            ...
            status_rows.append({
                ...
                "시장": row["market"],
                "상태": include_status,
                "수익률 관측치": info.get("observations", 0),
                "데이터 시작": (
